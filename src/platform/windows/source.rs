...
fn source_from_hwnd(window: HWND) -> Option<ProcessSource> {
    ...
    source_from_process_id(process_id)
}
...
fn source_from_process_id(process_id: u32) -> Option<ProcessSource> {
    ...
    let source = query_process_image_name(process).and_then(|path| source_from_image_path(&path));
    unsafe {
...
mod tests {
    ...
    use super::{source_from_hwnd, source_from_image_path, source_from_process_id};
    use std::ffi::OsString;
